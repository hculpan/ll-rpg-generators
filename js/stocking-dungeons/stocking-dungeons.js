...
function displayPrevious() {
    ...
    if (currentRoomDisplay > 0) {
        currentRoomDisplay--;
        $("#sd-main-result").html(roomContents[currentRoomDisplay]['output']);
        if (currentRoomDisplay == 0) {
            $("#sd-prev-btn").prop('disabled', true);
        }
    } else {
...
function storeResult(output, treasureRoll, roomRoll) {
    roomContents.push({
        output: output,
...
function replaceResult(output) {
    currentRoomDisplay = roomContents.length - 1;
    roomContents[currentRoomDisplay]['output'] = output;
    $("#sd-next-btn").prop('disabled', true);
    if (roomContents.length > 1) {
        $("#sd-prev-btn").prop('disabled', false);
    } else {
...
function stockRoom() {
    ...
    if (output != undefined && output != "") {
        storeResult(output, treasureRoll, roomContents);
    }
        ...
        query.find({
            success: function(results) {
                if (results.length == 0) {
                    ...
                    $("#sd-main-result").html(output);
                    storeResult(output, treasureRoll, roomContentsRoll);
...
function reRollMonster() {
    var currentRoom = roomContents[currentRoomDisplay];
    buildMonsterDungeonEncounter(currentRoom['roomRoll'], currentRoom['treasureRoll'],
        function(output, treasureRoll, roomRoll) {
...
function reRollSpecial() {
    var currentRoom = roomContents[currentRoomDisplay];
    var output = "<p>Special [roll=" + currentRoom['roomRoll'].toString() + "]</p>";
    output += "<p>" + buildSpecialRoomFeature() + "</p>";
    output += "<p>" + buildSpecialRoomCharacteristic() + "</p>";
    disableButtons('re-roll-special');
